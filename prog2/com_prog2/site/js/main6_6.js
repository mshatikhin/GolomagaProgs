$(document).ready(function (e) {
    var ed = $("#ed-6");	//ежемесяный доход
    var no = $("#no-6");	//налоговые отчисления
    var ap = $("#ap-6");	//арендная плата
    var svt = $("#svt-6");	//стоимость ввезенных товаров
    var zp = $("#zp-6");	//заработная плата работников
    var rt = $("#rt-6");	//расходы на транспортные услуги
    var pr = $("#pr-6");	//прочие расходы
    var pk = $("#pk-6");	//плата за комунальные услуги
    var pz = $("#pz-6");	//плата за кредит
    var itog1 = $("#itog1-6");	//чистая прибыль
    var nsm = $("#nsm-6");	//необходимая сумма на первый месяц
    var sums = $("#sums-6");	//общая сумма расходов
    var credit = $("#credit-6");	//кредит
    var ssn = $("#ssn-6");	//сумма средств в наличии
    var so = $("#so-6");	//стоимость оборудования
    var sr = $("#sr-6");	//стоимость рекламы
    var timeo = $("#timeo-6");	//срок окупаемости проекта, мес
    var srm = $("#srm-6");	//стоимость расходного материала
    var srm2 = $("#srm2-6");	//стоимость расходных материалов
    var kpp = $("#kpp-6");	//количество производимой продукции
    var dsp = $("#dsp-6");	//расходы связанные с производством
    var sebest = $("#sebest-6");	//себестоимость продукта
    var dobav = $("#dobav-6");	//добавленная стоимость
    var dohod = $("#dohod-6");	//доход с произведенной продукции


    function Calculate() {
        var sum = 0; var sum1 = 0; var sum3 = 0; var sum4 = 0; var sum5 = 0; var sum6 = 0; var sum7 = 0; var sum8 = 0;
        var s1i1 = !isNaN(parseFloat(ed.text().replace(".",","))) ? parseFloat(ed.text().replace(".",",")) : 0;
        ...
        sum = s1i3 + s1i4 + s1i13+ s1i6 + s1i8 + s1i9 + s1i10;
        sum1 = (s1i16 - ((s1i4 + s1i6 + s1i9+s1i13) / s1i14)) * s1i14 - sum;
        sum3 = (!isNaN(parseFloat(sum)) ? parseFloat(sum) :0) + s1i11 + s1i12;
        sum4 = (!isNaN(parseFloat(sum3)) ? parseFloat(sum3):0) - s1i2;
        sum5 = (!isNaN(parseFloat(sum3/sum1)) ? parseFloat(sum3/sum1):0);
        sum6 = s1i4 + s1i6 + s1i9;
        sum7 = (sum6+s1i13) / s1i14;
        sum8 = (s1i16 - sum7) * s1i14;
		...
		ed.text(sum8.toFixed(2));
    };

    function ClearAll() {
        $('#form6').find('input:text, input:password, input:file, textarea').val('');
        Calculate();
    };
    ...
    $("#reset-6").click(function () { ClearAll(); });    
